...
const ReportDetailModal = ({ report, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 animate-fade-in">
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-100 relative space-y-6 max-h-[92vh] overflow-y-auto font-sans">
        {/* Header */}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                ...
                {isEscalated
                  ...
                  : isValidated
                  ? "Validated & Verified"
                  : report.status || "Pending Barangay Review"}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Barangay</p>
        ...
        <div className="space-y-2">
          ...
          <div className="relative w-full h-56 rounded-2xl overflow-hidden border border-slate-300 shadow-inner">
        ...
        {report.reason && (
          <div className="p-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-xs text-amber-900 space-y-1">
            <p className="font-bold flex items-center gap-1.5">
              ...
              <span>Barangay Official Note:</span>
            </p>
            <p>{report.reason}</p>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          {photoUrl ? (
            <div className="w-full max-h-64 overflow-hidden rounded-2xl border border-slate-200 bg-slate-950/5 flex items-center justify-center p-2">
              <img
                src={photoUrl}
                alt="Photo Proof"
                className="max-h-60 w-auto rounded-xl object-contain shadow-sm"
              />
            </div>
          ) : (
